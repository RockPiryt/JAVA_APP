...
class WarGameTest {
    ...
    @Test
    void testPhaseWarDraw() {
        general1.recruitSoldier(Soldier.Rank.PRIVATE);
        general2.recruitSoldier(Soldier.Rank.PRIVATE);
        ...
        assertAll(
            // Sprawdza logi
            () -> assertTrue(secretary.getLogs().stream()
                    .anyMatch(log -> log.getAction().contains("Bitwa zakończyła się remisem")), "Powinno być zalogowane zakończenie bitwy remisem"),
            // Sprawdza stan armii
            () -> assertEquals(0, general1.getArmy().size(), "Armia Generała Alexandra powinna stracić jednego żołnierza"),
            () -> assertEquals(0, general2.getArmy().size(), "Armia Generała Philipa powinna stracić jednego żołnierza")
        );
    }
    ...
    void testLoggingAcrossPhases() {
        ...
        assertAll(
            () -> assertTrue(secretary.getLogs().stream()
                    .anyMatch(log -> log.getAction().contains("--- Runda 1 ---")), "Powinno być zalogowane rozpoczęcie rundy"),
            () -> assertTrue(secretary.getLogs().stream()
                    .anyMatch(log -> log.getAction().contains("Faza rekrutacji")), "Powinna być zalogowana faza rekrutacji"),
            () -> assertTrue(secretary.getLogs().stream()
                    .anyMatch(log -> log.getAction().contains("Faza manewrów")), "Powinna być zalogowana faza manewrów"),
            () -> assertTrue(secretary.getLogs().stream()
                    .anyMatch(log -> log.getAction().contains("Faza bitwy")), "Powinna być zalogowana faza bitwy"),
            () -> assertTrue(secretary.getLogs().stream()
                    .anyMatch(log -> log.getAction().contains("--- Koniec rundy 1 ---")), "Powinno być zalogowane zakończenie rundy")
        );
    }
